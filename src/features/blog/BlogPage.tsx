...
function PostCard({
  ...
  return (
    <div
      className={`group relative bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden flex flex-col hover:border-zinc-700 transition-colors ${featured ? "md:col-span-2 xl:col-span-1" : ""}`}
    >
      {/* Draft badge */}
        ...
        <div className="w-full aspect-video bg-zinc-800 overflow-hidden">
          {post.coverUrl ? (
            ...
            <div className="w-full h-full flex items-center justify-center">
              <span className="text-zinc-700 text-xs font-bold uppercase tracking-widest">
                No cover
              </span>
            </div>
          )}
      ...
      <div className="p-5 flex flex-col flex-1 gap-3">
        ...
        {post.tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {post.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="text-[9px] font-black uppercase tracking-widest text-violet-400 bg-violet-400/10 border border-violet-400/20 px-2 py-0.5 rounded-full"
              >
                {tag}
              </span>
            ))}
        ...
        {/* Excerpt */}
        {post.excerpt && (
          <p className="text-zinc-500 text-xs leading-relaxed line-clamp-2 flex-1 font-['Inter']">
            {post.excerpt}
          </p>
        )}
        ...
        <div className="flex items-center justify-between mt-auto pt-2 border-t border-zinc-800">
          <div className="flex items-center gap-2">
            {post.author?.profile?.imageUrl ? (
              ...
              <div className="w-5 h-5 rounded-full bg-zinc-700 flex items-center justify-center text-[9px]">
                👤
              </div>
            )}
